        ...
        public class Enumerator : IEnumerator<IList<T>> {
            ...
            public bool MoveNext() {
                if(this.myPosition == Position.BeforeFirst) {
                    ...
                    Array.Sort(this.myLexicographicalOrders);
                    ...
                    else if(this.NextPermutation() == false) {
                        this.myPosition = Position.AfterLast;
                    }
                }
            ...
            private bool NextPermutation() {
                ...
                int j = this.myLexicographicalOrders.Length;
                while(this.myLexicographicalOrders[j - 1] <= this.myLexicographicalOrders[i - 1]) {
                    --j;
                }
                this.Swap(i - 1, j - 1);
                ++i;
                j = this.myLexicographicalOrders.Length;
                while(i < j) {
                    this.Swap(i - 1, j - 1);
                    ++i;
                    --j;
        ...
        private void Initialize(IList<T> values, GenerateOption type, IComparer<T> comparer) {
            ...
            if(type == GenerateOption.WithRepetition) {
                ...
                if(this.myLexicographicOrders.Length > 0) {
                    this.myLexicographicOrders[0] = j;
                }
                for(int i = 1; i < this.myLexicographicOrders.Length; ++i) {
                    if(comparer.Compare(this.myValues[i - 1], this.myValues[i]) != 0) {
                        ++j;
                    }
                    this.myLexicographicOrders[i] = j;
                }
            }
