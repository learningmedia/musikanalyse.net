...
namespace PcSetTableGenerator.Combinatorics {
    using System;
    ...
    public class Variations<T> : IMetaCollection<T> {
        ...
        public class EnumeratorWithRepetition : IEnumerator<IList<T>> {
            ...
            public bool MoveNext() {
                ...
                if(this.myListIndexes == null) {
                    ...
                    for(int i = this.myListIndexes.Count - 1; i >= 0 && carry > 0; --i) {
                        this.myListIndexes[i] += carry;
                        carry = 0;
                        if(this.myListIndexes[i] >= this.myParent.UpperIndex) {
                            this.myListIndexes[i] = 0;
                            carry = 1;
                        }
                    }
                }
        ...
        public class EnumeratorWithoutRepetition : IEnumerator<IList<T>> {
            ...
            /// <returns>True if successfully moved to next variation, False if no more variations exist.</returns>
            public bool MoveNext() {
                bool ret = this.myPermutationsEnumerator.MoveNext();
                this.myCurrentList = null;
                return ret;
            }
            ...
            private void ComputeCurrent() {
                if(this.myCurrentList == null) {
                    this.myCurrentList = new List<T>();
                    int index = 0;
                    IList<int> currentPermutation = (IList<int>)this.myPermutationsEnumerator.Current;
                    for(int i = 0; i < this.myParent.LowerIndex; ++i) {
                    ...
                    for(int i = 0; i < currentPermutation.Count; ++i) {
                        ...
                        if(position != Int32.MaxValue) {
                            this.myCurrentList[position] = this.myParent.myValues[index];
        ...
        public long Count {
            get {
                if(this.Type == GenerateOption.WithoutRepetition) {
                    ...
                    return (long)Math.Pow(this.UpperIndex, this.LowerIndex);
                }
            }
        }
        ...
        private void Initialize(IList<T> values, int lowerIndex, GenerateOption type) {
            ...
            if(type == GenerateOption.WithoutRepetition) {
                ...
                this.myPermutations = new Permutations<int>(myMap);
            }
            else {
